...
def main():
    ...
    # print(initiate_room(auth({'login' : 'admin', 'password' : 'admin'})))
    ...
    while True:
        update = last_update(get_updates_json(bot_url))
        if (update['update_id'] != update_id):
            ...
            if (command['command'] == '/join'):
                ...
                if (uid not in tokensdb.keys()):
                    ...
                    try:
                        ...
                        if ('position' in response.keys()):
                            message_text = "There are " + str(response['position']) + " people before you"
                        else:
                            message_text = "Failed! an error occured\n" + response['err']
                    except Exception as e:
                        message_text = "Failed! an error occured\n" + str(e)
                ...
                send_mess(uid, 'You have left the room :)')
